...
use std::{cmp::min, env, error::Error, fs, io::{self, BufRead, Write}, path::{Path, PathBuf}, sync::mpsc::{self, Receiver, TryRecvError}, thread, time::Duration};
...
fn main() -> AppResult<()> {
    ...
    log.line(format!("loaded {} entries", entries.len()));

    let client = OpenAiClient::new(api_key)?;
    let mut h = Harness::new(entries, log);
    h.push_user(format!("Goal: {}\n\nUse log_action to inspect QuickLogger logs. Keep notes concise. Prefer small targeted pages. If the goal is unclear, use action=wait. When done, use action=finish.", cfg.goal));
    ...
    loop {
        ...
        if !p.messages.is_empty() { h.add_user_messages("Follow-up", p.messages); }
...
fn run_loop(h: &mut Harness, client: &OpenAiClient, input: &mut UserInput, model: &str, max_steps: usize, label: &str) -> bool {
    loop {
        match h.run(client, input, model, max_steps) {
            Ok(answer) => { h.log(format!("{label} finished")); println!("\n=== Answer ===\n{answer}\n\n=== Notes ===\n{}\n\n=== LLM Run Log ===\n{}", h.notes.trim(), h.log.path.display()); return true; }
            Err(e) => {
                h.log(format!("{label} paused after error: {e}"));
                eprintln!("\n=== Run paused after error ===\n{e}\n\nLLM run log: {}\n\nType /continue to retry, add context to retry with it, or /quit to exit.", h.log.path.display());
                let p = parse_controls(input.wait_for_messages());
                if p.quit || p.empty() { return false; }
                if !p.messages.is_empty() { h.add_user_messages("User input after error", p.messages); }
...
struct Config { goal: String, logs_path: PathBuf, model: String, max_steps: usize, verbose: bool }
impl Config {
    fn from_args() -> AppResult<Self> {
        let mut goal = None; let mut logs_path = env::var("QUICKLOGGER_LOGS_PATH").map(PathBuf::from).unwrap_or_else(|_| PathBuf::from(DEFAULT_LOGS)); let mut model = env::var("OPENAI_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.to_string()); let mut max_steps = MAX_STEPS; let mut verbose = false; let mut pos = Vec::new();
        let mut args = env::args().skip(1).peekable();
        while let Some(a) = args.next() { match a.as_str() { "--goal"|"-g" => goal = Some(next_arg(&mut args, "--goal")?), "--logs"|"-l" => logs_path = PathBuf::from(next_arg(&mut args, "--logs")?), "--model"|"-m" => model = next_arg(&mut args, "--model")?, "--max-steps" => max_steps = next_arg(&mut args, "--max-steps")?.parse()?, "--verbose"|"-v" => verbose = true, "--help"|"-h" => { print_help(); std::process::exit(0); }, other => pos.push(other.to_string()) } }
        let goal = goal.or_else(|| (!pos.is_empty()).then(|| pos.join(" "))).ok_or("Pass a goal with --goal \"...\"")?;
...
fn print_help() { eprintln!("cargo run --bin log_llm -- --goal \"summarise my logs\" [--model gpt-5.4-mini] [--verbose]"); }
...
struct RunLog { path: PathBuf, file: fs::File, verbose: bool }
impl RunLog {
    fn new(verbose: bool) -> AppResult<Self> { fs::create_dir_all(LOG_DIR)?; let path = Path::new(LOG_DIR).join(format!("{}_{}.log", Utc::now().format("%Y%m%dT%H%M%SZ"), std::process::id())); let mut file = fs::OpenOptions::new().create(true).append(true).open(&path)?; writeln!(file, "# QuickLogger compact LLM run\n---")?; Ok(Self { path, file, verbose }) }
...
struct Harness { entries: Vec<Entry>, cursor: Option<usize>, notes: String, summary: String, log: RunLog, input: Vec<Value> }
impl Harness {
    ...
    fn run(&mut self, client: &OpenAiClient, user_input: &mut UserInput, model: &str, max_steps: usize) -> AppResult<String> { for step in 0..max_steps { self.drain_user(user_input)?; self.compact_history(); self.log(format!("step {}/{}: model request with {} items, approx {} chars", step+1, max_steps, self.input.len(), value_chars(&self.input))); let body = json!({"model":model,"service_tier":SERVICE_TIER,"store":false,"input":self.input.clone(),"instructions":self.instructions(),"tools":[self.tool()],"parallel_tool_calls":false}); self.log.value("openai_request", &body); let resp = client.response(&body, &mut self.log)?; self.log.value("openai_response", &resp); let output = resp.get("output").and_then(Value::as_array).cloned().unwrap_or_default(); let calls: Vec<Value> = output.iter().filter(|i| i.get("type").and_then(Value::as_str)==Some("function_call")).cloned().collect(); self.push_output_items(&output); if calls.is_empty() { let text = output_text(&resp); if text.trim().is_empty() { return Err("model produced no output".into()); } return Ok(text); } for call in calls { let call_id = call.get("call_id").and_then(Value::as_str).ok_or("missing call_id")?.to_string(); let args_s = call.get("arguments").and_then(Value::as_str).unwrap_or("{}"); let args: Value = serde_json::from_str(args_s).unwrap_or_else(|_| json!({})); let action = args.get("action").and_then(Value::as_str).unwrap_or(""); self.log(format!("log_action: {action} {}", trunc(args_s, 500))); if action == "finish" { let answer = args.get("answer").and_then(Value::as_str).unwrap_or("").trim().to_string(); if answer.is_empty() { return Err("finish missing answer".into()); } self.input.push(func_output(&call_id, &json!({"ok":true}))); return Ok(answer); } let result = if action == "wait" { self.wait(&args, user_input)? } else { self.action(&args) }; self.log_result(action, &result); self.log.value("tool_result", &json!({"action":action,"result":result.clone()})); self.input.push(func_output(&call_id, &result)); } } Err(format!("Reached --max-steps ({max_steps})").into()) }
    ...
    fn instructions(&self) -> String { format!("You analyze QuickLogger logs. There are {} parsed entries. Use one tool: log_action. Keep requests targeted and page through results. Available actions: summary, get_index, previous, next, day, between, search, around, read_notes, write_notes, wait, finish. Dates are UTC YYYY-MM-DD. Tool outputs are intentionally compact; request another page or surrounding context if needed.", self.entries.len()) }
    fn tool(&self) -> Value { json!({"type":"function","name":"log_action","description":"Inspect QuickLogger logs, update notes, wait for user, or finish.","parameters":{"type":"object","properties":{"action":{"type":"string","enum":["summary","get_index","previous","next","day","between","search","around","read_notes","write_notes","wait","finish"]},"index":{"type":"integer"},"from_index":{"type":"integer"},"date":{"type":"string"},"start_date":{"type":"string"},"end_date":{"type":"string"},"query":{"type":"string"},"case_sensitive":{"type":"boolean"},"before":{"type":"integer"},"after":{"type":"integer"},"limit":{"type":"integer"},"offset":{"type":"integer"},"mode":{"type":"string","enum":["append","replace"]},"text":{"type":"string"},"question":{"type":"string"},"reason":{"type":"string"},"answer":{"type":"string"}},"required":["action"],"additionalProperties":false}}) }
    fn action(&mut self, args: &Value) -> Value { match args.get("action").and_then(Value::as_str).unwrap_or("") { "summary" => self.summary_action(), "get_index" => self.get_index(args), "previous" => self.prev(args), "next" => self.next(args), "day" => self.day(args), "between" => self.between(args), "search" => self.search(args), "around" => self.around(args), "read_notes" => json!({"notes":self.notes}), "write_notes" => self.write_notes(args), other => json!({"error":format!("unknown action {other}")}) } }
    fn summary_action(&self) -> Value { let mut counts = Vec::<(String, usize)>::new(); for e in &self.entries { match counts.last_mut() { Some((d,c)) if *d==e.date_utc => *c += 1, _ => counts.push((e.date_utc.clone(),1)) } } json!({"entry_count":self.entries.len(),"cursor_index":self.cursor,"first":self.entries.first().map(|e| self.entry(e)),"last":self.entries.last().map(|e| self.entry(e)),"counts_by_day":counts.into_iter().map(|(date,count)|json!({"date":date,"count":count})).collect::<Vec<_>>()}) }
