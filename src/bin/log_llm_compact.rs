...
use std::{cmp::min, collections::HashSet, env, error::Error, fs, io::{self, BufRead, Write}, path::{Path, PathBuf}, sync::mpsc::{self, Receiver, TryRecvError}, thread, time::{Duration, SystemTime}};
...
struct Config { goal: Option<String>, logs_path: PathBuf, model: String, max_steps: usize, verbose: bool, resume: Option<PathBuf>, resume_latest: bool }
impl Config {
    fn from_args() -> AppResult<Self> {
        let mut goal = None; let mut logs_path = env::var("QUICKLOGGER_LOGS_PATH").map(PathBuf::from).unwrap_or_else(|_| PathBuf::from(DEFAULT_LOGS)); let mut model = env::var("OPENAI_MODEL").unwrap_or_else(|_| DEFAULT_MODEL.to_string()); let mut max_steps = DEFAULT_MAX_STEPS; let mut verbose = false; let mut resume = None; let mut resume_latest = false; let mut pos = Vec::new();
        ...
        Ok(Self { goal, logs_path, model, max_steps, verbose, resume, resume_latest })
    }
